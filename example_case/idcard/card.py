...
class PaddleOcrImpl(Ocr):
    def __init__(self):
        self.ocrTarget = PaddleOCR(use_angle_cls=True, lang="ch")
    def ocr(self,image):
        result = self.ocrTarget.ocr(image, cls=True)
        return result
    def ocr_for_single_line(self,image):
        result=self.ocr(image)
        return result[0] if len(result)>0 else None
class Card:
    ...
    """
    def getCardNoLocation(self,img=None):
        if img is None:
            img=self.img
        grayImg=cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        # 二值化处理，将黑字变成白色，偏白色区域转换成黑色
        _, thresholdImg = cv2.threshold(grayImg, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
...
class Yolov5Card(Card):
    ...
    @staticmethod
    def confYolov5(yolov5Path, ptPath,facePtPath):
    ...
    def getCardImg(self):
        #被截取图像的位置
        cobj = self.getCardLocation()
        if cobj is not None:
            dst = self.img.copy()
            x, y, w, h,labels = cobj
            crop_img = dst[y:y + h, x:x + w]
            # grayImg = cv2.cvtColor(crop_img, cv2.COLOR_BGR2GRAY)
            # crop_img = cv2.threshold(grayImg, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]
            # crop_img = cv2.resize(crop_img, (472,354), interpolation=cv2.INTER_AREA)
    ...
    def get_data_areas(self, img=None,label=None):
        return img,label
    """
        对每个区域进行ocr获取到身份证的各部分数据
    """
    from paddleocr import PaddleOCR, draw_ocr
    def ocrArea(self, areas,label):
        rowObj = {}
        areasCopy=areas.copy()
        result = self.ocr.ocr(areas)
        maxH = 0
        noLocation = None
        #通过区域获取图像倾斜的角度，并纠正
        for idx in range(len(result)):
            ...
            for line in res:
                ...
                # cv2.polylines(areasCopy, [np.array(pts)], True, (0, 255, 0), 2)
                ...
                if maxHeight > maxH:
                    maxH = maxHeight
                    noLocation = pts
        rotated_img, M = common.justImage(areasCopy, noLocation)
        common.show(cv2.cvtColor(rotated_img, cv2.COLOR_BGR2RGB), "纠正图像", debug=True)
        areas=[]
        #通过纠正图形获取到最新的坐标
        for idx in range(len(result)):
            res = result[idx]
            for line in res:
                ...
                # line[0]=pointsNew
                (x,y,w,h)=common.getNonmal(pointsNew)
                areas.append((x,y,w,h,line[1][0]))
        ...
        hre=0
        row3MinX=None
        result = {"姓名": "", "性别": "", "民族": "", "住址": "","success":True,"rotated_img":rotated_img}
        if "idcard_z" == label:
            rowObj = {}
            # 先按照行归类
            for index, rect in enumerate(areas):
                ...
                if not row in rowObj:
                    rowObj[row] = []
                rowObj[row].append((x, y, w, h,value))
                # print("第",row,"行，元素",(x, y, w, h,data_area))
                if index + 1 < len(areas):
                    if row==0:
                        hre=0.3*h
                    if row==3:
                        if row3MinX is None:
                            row3MinX=x
                        if areas[index + 1][0]-row3MinX<15:
                            row = 1 + row
                        #可能身份證號碼在稍微上翹可能导致身份证号码加入到地址中，判断数字
                        nextChar=areas[index + 1][4]
                        nextCharLen=len(nextChar)
                        if nextCharLen>12 and nextChar[0:nextCharLen-1].isdigit() :
                            row = 1 + row
                    else:
                        if areas[index + 1][1] - y-h > hre:
                            row = 1 + row
                        # hre=areas[index + 1][1] - y-h
        # print("row",rowObj)
            common.printPretty(rowObj)
            if len(rowObj.keys()) < 5:
                result["success"] = False
                return result
            # 解析第一行，姓名
            rowObj0Array = sorted(rowObj[0], key=lambda x: x[0])
            for row0Index, rowObj0 in enumerate(rowObj0Array):
                x, y, w, h, text = rowObj0
                text=text.lstrip('姓名')
                if len(rowObj0Array) > 1 and row0Index == 0 and (
                        text.find("姓") >= 0 or text.find("名") >= 0):  # 过滤掉姓名，错误的：欢名
                    continue
                if text in ['姓名', '名'] or len(text) > 6:
                    continue
                result["姓名"] += text
            rowObj1Array = sorted(rowObj[1], key=lambda x: x[0])
            for rowObj1 in rowObj1Array:
                x, y, w, h, text = rowObj1
                text=text.lstrip('性别')
                textArray=text.split("民族")
                for text in textArray:
                    if text in ["男", "女"]:
                        result["性别"] = text
                    if text in ['安', '如', '妙', '妍', '妤', '妞', '妹', '妻', '姊', '姐', '姒', '姓', '委', '娃', '娅', '娇', '娜', '娟',
                                '姣', '姘', '娥']:
                        result["性别"] = '女'
                    if text.startswith("族") or text.startswith("民族"):
                        result["民族"] = text.split("族")[1]
                    nations = ['汉', '壮', '满', '回', '苗', '维吾尔', '土家', '彝', '蒙古', '藏', '布依', '侗', '瑶', '朝鲜', '白', '哈尼', '黎',
                               '哈萨克', '傣', '畲', '傈僳', '东乡', '仡佬', '拉祜', '佤', '水', '纳西', '羌', '土', '仫佬', '锡伯', '柯尔克孜', '景颇',
                               '达斡尔', '撒拉', '布朗', '毛南', '塔吉克', '普米', '阿昌', '怒', '鄂伦春', '赫哲', '门巴', '珞巴', '基诺', '德昂', '保安',
                               '裕固',
                               '京族', '塔塔尔', '独龙', '鄂温克']
                if text in nations:
                    result["民族"] = text
